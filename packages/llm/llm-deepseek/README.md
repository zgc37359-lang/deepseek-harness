...
- **Thinking-mode models occasionally stream CoT into `delta.content`** — translate routes a content stream that begins with `<thinking>` into the reasoning block (cross-fragment tags, deduped against `reasoning_content`); serialize strips leading `<thinking>` segments on replay. Mid-text literals stay text, and the mirrored echo order (content-sourced thinking first, then an identical `reasoning_content` echo) is not deduplicated.
